...
public:
    virtual void SetUp() {
        ...
        auto now_stamp = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
        {
            ...
            entry->set_application("credentials_test_usig_session_list");
...
TEST_F(credentials_test, user_password_success) {
    std::string command;

    command = "tgctl credentials ";
    command += helper_->abs_path("credentials.key");
    command += " --user tsurugi --verbose --conf ";
    command += helper_->conf_file_path();

    std::stringstream ss;
    do_test(command, "password\n", ss);

    command = "tgctl session list --credentials ";
    command += helper_->abs_path("credentials.key");
