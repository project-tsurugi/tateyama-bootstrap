...
template<>
inline void server_mock::request_message<tateyama::proto::request::request::ExtractStatementInfo>(tateyama::proto::request::request::ExtractStatementInfo& rq) {
    tateyama::proto::request::request::Request r{};
    auto s = current_request();
    EXPECT_TRUE(r.ParseFromString(s));
    EXPECT_EQ(r.command_case(), tateyama::proto::request::request::Request::CommandCase::kExtractStatementInfo);
...
TEST_F(request_test, request_list) {
    std::array<std::array<std::size_t, 5>, 3> expected = {
        {
            {123, 1, 4, 13, 98765},
            {234, 2, 5, 24, 87654},
            {345, 3, 6, 36, 76543},
...
//    std::cout << result << std::flush;
    ...
    EXPECT_NE(std::string::npos, result.find("             98765"));
    ...
    EXPECT_NE(std::string::npos, result.find("             87654"));
    ...
    EXPECT_NE(std::string::npos, result.find("             76543"));
...
TEST_F(request_test, request_extract_sql) {
    {
        tateyama::proto::request::response::ExtractStatementInfo response{};
        auto* success = response.mutable_success();
    ...
    command = "tgctl request extract-sql YWJjZGVmZw --conf ";
    command += helper_->conf_file_path();
    std::cout << command << std::endl;
    if((fp = popen(command.c_str(), "r")) == nullptr){
...
//    std::cout << result << std::flush;
    ...
    EXPECT_EQ(tateyama::framework::service_id_request, server_mock_->component_id());
    tateyama::proto::request::request::ExtractStatementInfo rq{};
    server_mock_->request_message(rq);
